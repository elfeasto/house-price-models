...
y = train_data["price"]
for num_nn in range(10,201,2):
    sklearn_k_nn = KNeighborsRegressor(n_neighbors=num_nn)
    CVS = cross_val_score(sklearn_k_nn, train_data[["sqft_living"]], train_data["price"], cv=10)
    print(num_nn, "   ", np.round(CVS.mean(),5))
